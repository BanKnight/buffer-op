...
box.pack_error = function (writer, target)
{
    writer.append_uint8(types.err)

    let keys = Object.getOwnPropertyNames(target)

    writer.append_uint16(keys.length)
...
box.unpack_err = function (reader)
{
    let len = reader.read_uint16()
    let ret = Error("")
